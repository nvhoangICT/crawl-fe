...
export const ListRestaurant: React.FC<Props> = ({ data, isLoading, limit }) => {
  ...
  const [dataUpdate, setDataUpdate] = useState<RestaurantFormData | null>(null);

  const handleDelete = async (documentDeleteId: string) => {
    try {
      setIsLoadingDelete(true);
      await restaurantService.delete(documentDeleteId);
              ...
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center">
                    <EmptyData title="Chưa có nhà hàng nào" />
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
