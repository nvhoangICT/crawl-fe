...
import CreateOrUpdateLandmark from "./CreateOrUpdateLandmark";
...
export const ListLandmark: React.FC<Props> = ({ data, isLoading, limit }) => {
  ...
  const [dataUpdate, setDataUpdate] = useState<Landmark | null>(null);
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast.success("Xóa địa điểm tham quan thành công");
      setIsRefresh(!isRefresh);
    } catch (error) {
      console.log(error);
      toast.error("Xóa địa điểm tham quan thất bại");
    } finally {
  ...
  return (
    <div>
              ...
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center">
                    <EmptyData title="Chưa có địa điểm tham quan nào" />
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
      ...
      <DialogConfirm
        ...
        title="Xác nhận xóa địa điểm tham quan"
        description="Bạn có chắc chắn muốn xóa địa điểm tham quan này không?"
