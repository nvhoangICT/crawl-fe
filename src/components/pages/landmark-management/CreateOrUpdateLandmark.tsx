...
import { landmarkSchema } from "@/schemas/landmarkSchema";
...
interface Props {
  onClose?: () => void;
  open?: boolean;
  dataUpdate?: Landmark | null;
  readonly?: boolean;
}

const CreateOrUpdateLandmark = ({
  open,
  onClose,
  dataUpdate,
  readonly,
  ...
  const onFormSubmit = async (data: landmarkFormData) => {
    try {
      if (dataUpdate) {
        // Update existing restaurant
        await spaService.update(dataUpdate.id, data);
        toast.success("Cập nhật địa điểm tham quan thành công!");
      } else {
        // Create new restaurant
        await spaService.create(data);
        toast.success("Thêm địa điểm tham quan mới thành công!");
      }
      form.reset();
      onClose?.();
      ...
      toast.error("Đã xảy ra lỗi khi lưu dữ liệu địa điểm tham quan!");
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[60vw] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {dataUpdate ? "Chỉnh sửa địa điểm tham quan" : "Thêm địa điểm tham quan mới"}
          </DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form
...
export default CreateOrUpdateLandmark;